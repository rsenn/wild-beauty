...
(async () => {
  var api = new API();
  const fields = ["photos { photo { id width height filesize } }", "users { user { id email last_seen } }"];
  let r = await api.select("items", { parent_id: 1 }, fields);
const items = (await r.items).map(i => i);

  console.log("r = ", util.inspect(items, { depth: 4 }));
