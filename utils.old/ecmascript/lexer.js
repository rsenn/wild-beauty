...
l.addToken = function(type) {
  const { start, pos, column, line } = this;
  const token = new Token(type, this.source.substring(this.start, this.pos), this.start, this.pos, { line, column });
...
l.lexPunctuator = function() {
  ...
  if(isPunctuator(word)) {
    ...
    // catch it if it does.
    throw new SyntaxError(this.position() + 'Invalid punctuator: ' + word);
  }
};
...
l.lexText = function() {
  do {
    ...
    if(c === null) {
      ...
      throw new SyntaxError(this.line + ':' + this.column + ' Unexpected character: ' + c);
    }
  } while(true);
};
