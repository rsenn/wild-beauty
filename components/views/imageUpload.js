          ...
          {images.map((image, index) => {
            ...
            return (
              <div key={index} className={"image-entry tooltip"} data-tooltip={`${width}x${height} ${orientation}`}>
                ...
                <button className={"image-button image-rotate center-flex"} onClick={() => onRotate(id)}>
                  <svg height="24" width="24" viewBox="0 0 16 16">
                    <defs />
                    <g transform={`translate(7.483179807662964, 0)`}>
                      <g transform={` scale(${shift ? -1 : 1}, 1)`}>
                        <g transform={`translate(-7.483179807662964, 0)`}>
                          <path d="m 11.993 10.754 a -6.934 -5.695 0 0 1 -3.176 2.27 a -3.761 -7.968 0 0 1 -3.84 -0.708 a 0.074 -7.265 0 0 1 -2.105 -3.006 a 2.179 -4.259 0 0 1 0.517 -3.634 h 0 a 1.58 -0.707 0 0 1 2.939 -2.586 a -1.358 1.88 0 0 1 3.895 0.39 a -5.283 1.46 0 0 1 2.005 2.11" fill="none" stroke="#00f" strokeWidth="1.66983039" />
                          <path d="M9.434 5.852l4.516 2.14-.405-4.981-4.11 2.84z" fill="#00f" />
                        </g>
                      </g>
                    </g>
                  </svg>
